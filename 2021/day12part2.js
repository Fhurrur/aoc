...
function visit(node, path, visitedSmallCaveAlready) {
    ...
    return nodes[node].nodes
        ...
        .flatMap(node => {
            let smallAndVisited = !nodes[node].big && path.includes(node);
            if (smallAndVisited && visitedSmallCaveAlready) {
                return [];
            }
            return [...visit(node, localPath, smallAndVisited || visitedSmallCaveAlready)];
        })
}

console.log(visit('start', [], false).length)
